...
public class LibraryManagement {
	...
	static void displayAllBooks() {
		...
		System.out.println("Book ID" + "  " + "Book Title         " + "LendFlag");
				...
				filter_member.stream().forEach(data -> {
					if (data.getMemberId() == memberId) {
						...
						if (custom_value == "lendBook") {
							...
							if (data.getBorrowedBooks().contains(book_id)) {
								data.getBorrowedBooks().removeIf((s -> s.equals(book_id)));
							} else {
								System.out.println("Borrowed Book not exists");
							}
