...
public class Machine1 {
	...
	static void checkItemNo(List<ItemInfo> filter_array, int number) {
		long itemCount = filter_array.stream().filter(data -> data.getItemno() == number).count();
		if (itemCount == 0) {
	...
	static void calculatePrice() {
		String inputitem;
		try {
			filter_array = itemList.stream().filter(data -> data.getPrice() <= enterprice).collect(Collectors.toList());
			BufferedReader reader = new BufferedReader(new InputStreamReader(System.in));
			System.out.println("Enter Item");

			inputitem = reader.readLine();
